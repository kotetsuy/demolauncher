...
1. **同時に 1 つのデモしか動かない** — デモ間でポートが重複しているため
   （`:8000` は 4 デモ、`:8080` は 6 デモが取り合う）、起動前に必ず全デモを停止する
...
| 3dslam3 | `~/3dslam3` | 8080 | 起動スクリプトの正常終了（ROCm準備完了） | 210s |
...
> **3dslam3 について**: `start_all.sh` が `/api/health` と ROCm・DA3 の
> ウォームアップ完了を確認するため、正常終了を待ってから既定ブラウザで
> `http://127.0.0.1:8080/` を開く。既定ポート 8080 を使用する
> （起動環境で `PORT` を変更しないこと）。
...
- **全て停止** — 8 デモすべてに `stop_all.sh` を流す。1 つ失敗しても残りは止め、
  失敗したデモ名をまとめて表示する
